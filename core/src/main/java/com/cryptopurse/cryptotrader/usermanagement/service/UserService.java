package com.cryptopurse.cryptotrader.usermanagement.service;

import com.cryptopurse.cryptotrader.email.service.MailService;
import com.cryptopurse.cryptotrader.infrastructure.exception.CryptopurseException;
import com.cryptopurse.cryptotrader.usermanagement.domain.CryptotraderUser;
import com.cryptopurse.cryptotrader.usermanagement.domain.Role;
import com.cryptopurse.cryptotrader.usermanagement.domain.RoleEnum;
import com.cryptopurse.cryptotrader.usermanagement.repository.RoleRepository;
import com.cryptopurse.cryptotrader.usermanagement.repository.UserRepository;
...
public class UserService {
    ...
    @Value("${com.cryptopurse.fullbaseurl}")
    ...
    public CryptotraderUser registerByInvitation(String email) {
        ...
        if (alreadyInDatabase.isPresent()) {
            throw new CryptopurseException("That user is already registered. Unable to register by invitation");
        } else {
    ...
    public void forgotPassword(String email) {
        ...
        if (byEmail.isPresent()) {
            ...
            mailService.createMail()
                    .body(bodyBuilder.toString())
                    .subject("Cryptopurse | Password Reset")
    ...
    public void changePassword(CryptotraderUser cryptotraderUser, String oldPassword, String password) {
        ...
        if (one.isPresent()) {
            ...
            if (standardPasswordEncoder.matches(oldPassword, fetchedUser.getPassword())) {
                ...
                throw new CryptopurseException("Your current password was incorrect");
            }
        }
    }
    ...
    public void setPasswordForActivationCode(String activationCode, String email, String password) {
        ...
        if (byActivationCode.isPresent()) {
            if (!byActivationCode.get().getEmail().equals(email)) {
                throw new CryptopurseException("That email is not associated with this activation code");
            } else {
                userRepository.save(
                        byActivationCode.get()
                                .setPassword(standardPasswordEncoder.encode(password))
